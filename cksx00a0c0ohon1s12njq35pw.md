...
You can go several ways to set up the configuration. You can create a ``doctrine.global.php`` config file and overwrite
...
The Doctrine Console is a very useful command line interface tool. However, it doesn't work out of the box. Doctine needs to know how to set up the entity manager with the right configuration. Fortunately, this is easily done by creating the file ``cli-config.php`` in the config dir.
...
This file loads the configuration, set up the container and gets the entity manger which will then be injected in the
