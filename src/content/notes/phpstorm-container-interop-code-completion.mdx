...
containers. These containers used the `Interop\Container\ContainerInterface` and since February 2017
it's accepted by PHP-Fig as PSR-11 and use the `Psr\Container\ContainerInterface`. I know there is a
...
suddenly showed up. It turns out be be really easy. All you need is a `.phpstorm.meta.php` file in
...
The magic happens here: `'' == '@'`. It tries to resolve anything to a valid class. So now you can
do this:
...
`'logger' instanceof \Psr\Log\LoggerInterface`. Once it is added you also have auto completion for
that:
...
And otherwise you can most likely change the config file a bit and make it work for the dependency
container you use. More info on how to use this can be found on the
